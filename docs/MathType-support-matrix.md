...
| `TM_LDIV` | 26 | partial (header only) | `writeLongDivisionHeader` | `writeLongDivisionNode / writeCompositeLongDivisionByTemplate` |
...
- Complete long division is not implemented. `TM_LDIV` currently preserves only the quotient/dividend template with the divisor shell outside it. A following `array` is an ordinary adjacent caller-supplied structure, not validated long-division semantics. Automatic subtraction rows, digit carry-down, underline alignment, and remainder placement are unsupported.
