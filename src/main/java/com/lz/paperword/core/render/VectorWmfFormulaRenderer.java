...
final class VectorWmfFormulaRenderer {
    ...
    private static FormulaLayout layoutUnderline(String text) {
        return layoutUnderline(text, 0);
    }

    private static FormulaLayout layoutUnderline(String text, int sqrtDepth) {
        ...
        FormulaLayout inner = layoutFractionPart(text.substring(groupStart + 1, groupEnd), sqrtDepth);
        if (inner == null) {
            return null;
        }
        List<PlacedText> placed = new ArrayList<>();
    ...
    private static FormulaLayout layoutCancel(String text) {
        return layoutCancel(text, 0);
    }

    private static FormulaLayout layoutCancel(String text, int sqrtDepth) {
    ...
    private static FormulaLayout layoutSqrt(String text, int sqrtDepth, boolean insideFractionSlot) {
        ...
        while (cursor < text.length()) {
            ...
            double bodyScale = compactBodyFraction
                ? sqrtBodyFractionScale(sqrtDepth) : 1.0d;
            ...
            boolean compactNestedFractionBody = compactBodyFraction && sqrtDepth > 0;
            if (compactNestedFractionBody) {
                bodyPad += MathTypeStructureMetrics.SQRT_NESTED_BODY_FRACTION_LEFT_EXTRA_PT;
            }
            ...
            if (tallRoot) {
                ...
                double ordinaryTopLeadX = MathTypeStructureMetrics.SQRT_TALL_CHECK_TOP_LEAD_X_PT;
                double ordinaryTopLeadYRatio = MathTypeStructureMetrics.SQRT_TALL_CHECK_TOP_LEAD_Y_RATIO;
                double ordinaryUpperTransitionX =
                    MathTypeStructureMetrics.SQRT_TALL_CHECK_UPPER_TRANSITION_X_PT;
                double ordinaryUpperTransitionYRatio =
                ...
                double upperTransitionX = MathTypeStructureMetrics.SQRT_TALL_COMPACT_CHECK_UPPER_TRANSITION_X_PT;
                double upperTransitionYRatio =
                ...
                if (compactBodyFraction) {
                    lines.add(LineSegment.rootPolyline(
                        ...
                        rootX + upperTransitionX,
                        rootHeight * upperTransitionYRatio,
                        rootX + shoulderX,
                    ...
                    lines.add(LineSegment.rootPolyline(
                        ...
                        rootX + checkMidX,
                        rootHeight - MathTypeStructureMetrics.sqrtBottomPadPt(rootHeight),
                        rootX + ordinaryUpperTransitionX,
                        rootHeight * ordinaryUpperTransitionYRatio,
                        rootX + shoulderX,
                        rootHeight * shoulderYRatio,
                        rootX + ordinaryTopLeadX,
                        rootHeight * ordinaryTopLeadYRatio,
                        rootX + checkTopX, MathTypeStructureMetrics.SQRT_TOP_Y_PT,
                        topBarEnd, MathTypeStructureMetrics.SQRT_TOP_Y_PT
                    ));
                }
    ...
    private static double sqrtBodyFractionScale(int sqrtDepth) {
        return sqrtDepth > 0 ? MathTypeStructureMetrics.SQRT_NESTED_BODY_FRACTION_SCALE
            : MathTypeStructureMetrics.SQRT_BODY_FRACTION_SCALE;
    }
